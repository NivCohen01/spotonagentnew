...
# Kept for compatibility with your regrouping logic (currently unused).
...
async def _save_workspace_domain_if_applicable(sess: "Session", email_addr: str, password: str) -> None:
    """
    ...
    domain_source = getattr(sess, "start_url", "") or ""
...
def _candidate_mailbox_passwords(sess: "Session", email_addr: str, preferred_password: Optional[str] = None) -> list[str]:
    """
...
def _register_mailbox_actions(tools: Tools, sess: "Session") -> None:
    log = logging.getLogger("service")
    ...
    async def create_signup_mailbox(params: CreateMailboxParams):
        base_source = params.base_hint or params.target_url or sess.start_url or sess.task or ""
        ...
        if sess.generated_credentials:
            ...
            if existing_email and existing_password:
                log.info("Reusing generated mailbox %s for session %s", existing_email, sess.id)
                return ActionResult(
    ...
    async def fetch_mailbox_otp(params: FetchOTPParams):
        chosen_email = params.email or (sess.user_credentials or {}).get("email") or (sess.generated_credentials or {}).get("email")
