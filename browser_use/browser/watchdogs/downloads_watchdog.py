...
class DownloadsWatchdog(BaseWatchdog):
    ...
    _session_manager_missing_logged: bool = PrivateAttr(default=False)  # Avoid repeated warnings when session manager is gone
                ...
                def on_response_received(event: ResponseReceivedEvent, session_id: str | None) -> None:
                    ...
                    try:
                        ...
                        if not self.browser_session.session_manager:
                            if not self._session_manager_missing_logged:
                                self.logger.debug('[DownloadsWatchdog] Session manager not found, skipping network monitoring')
                                self._session_manager_missing_logged = True
                            return
                        else:
                            # Reset flag once session manager becomes available again
                            if self._session_manager_missing_logged:
                                self._session_manager_missing_logged = False
